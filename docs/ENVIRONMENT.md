...
**Key Operational Workflows:** \* **Intelligent Sync**: Implements a 3-way merge logic to prevent accidental overwrites of remote-only variables. \* **Security Layer**: Every remote write operation automatically triggers a `chmod 600` to protect secrets. \* **Secret Masking**: The `cat` and `diff` commands pass content through a masking filter that replaces sensitive patterns (API keys, tokens) with `***`.
...
The `DockerManager` leverages the `--env-file` flag of the Docker CLI. \* **Runtime**: `docker run --env-file .env ...` \* **Compose**: `docker-compose --env-file .env up` \* **Generation**: The system can export the current `VLLMConfig` directly into a Docker-compatible `.env` file.

------------------------------------------------------------------------

## Best Practices & Security

> \[!IMPORTANT\] **Never commit `.env` files to version control.** Your `.gitignore` is pre-configured to exclude all `.env*` files.
