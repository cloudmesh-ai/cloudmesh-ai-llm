...
Request a 4-GPU interactive session. Replace `<node-id>` with your assigned node ID in the following steps.
...
- **Mount source doesn't exist:** Run `mkdir -p /scratch/${USER}/hf_cache` on the host before launching.

- **Connection Refused:** Verify the tunnel is pointing to the correct `<node-id>` node where the job is active.
