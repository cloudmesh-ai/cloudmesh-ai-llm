...
@click.option(
    "--docker",
    is_flag=True,
    help="Run Aider in a Docker container to avoid Python version issues",
)
...
def init_server(host):
    ...
    if "cloudmesh" not in data:
        data["cloudmesh"] = {}
    if "ai" not in data["cloudmesh"]:
        data["cloudmesh"]["ai"] = {}
    if "server" not in data["cloudmesh"]["ai"]:
        data["cloudmesh"]["ai"]["server"] = {}

    # Determine platform and defaults based on host
    platform = (
        "uva"
        if "uva" in host.lower()
        else "dgx" if "dgx" in host.lower() else "default"
    )
    server_name = f"gemma-{host}"

    if server_name in data["cloudmesh"]["ai"]["server"]:
        if not console.ynchoice(
            f"Server {server_name} already exists. Overwrite?", default=False
        ):
            return
    ...
    try:
        ...
        console.ok(
            f"Initialized server '{server_name}' (platform: {platform}) in {config_path}"
        )
    except Exception as e:
...
def config_info():
    ...
    try:
        ...
        if not resolved_yaml or resolved_yaml == "{}":
            console.warning(
                "The in-memory configuration is empty. This may be due to a loading error or an empty config file."
            )
        else:
            console.print(
                banner(
                    "In-Memory Resolved Configuration",
                    f"Source: {orchestrator.config_path}\n\n{resolved_yaml}",
                )
            )
...
@llm_group.command(name="start", context_settings=dict(allow_intermixed_options=True))
...
@click.option(
    "--export",
    is_flag=True,
    help="Export launch scripts to local directory for customization",
)
    ...
    try:
        ...
        if export:
            ...
            if orchestrator.export_scripts(name):
                console.ok(
                    "Scripts exported successfully. You can now edit them locally."
                )
            else:
        ...
        if isinstance(clients, dict) and name in clients:
            ...
            console.print(
                banner(
                    f"Launching Client: {name}",
                    f"Host: {client_config.get('host')}\nPort: {client_config.get('port')}\nLauncher: {launcher_name}",
                )
            )
            ...
            if launcher_class:
                ...
                console.error(
                    f"Unsupported or missing launcher '{launcher_name}' for client '{name}'."
                )
            return
        ...
        if orchestrator.prepare_backend(name, port_override=port):
            ...
            if ui:
                ...
                clients = orchestrator.db.get("cloudmesh.ai.client", {})
                webui_cfg = (
                    clients.get("openwebui", {}) if isinstance(clients, dict) else {}
                )
                ...
                clients = orchestrator.db.get("cloudmesh.ai.client", {})
                claude_cfg = (
                    clients.get("claude", {}) if isinstance(clients, dict) else {}
                )
                ...
                console.banner(
                    label="llm service started", txt=banner_text, color="dark_green"
                )

                console.msg(
                    "Backend is ready. You can now run 'cmc launch webui' or 'cmc launch claude'."
                )
                console.print(
...
@click.option(
    "--port", type=str, help="Port or partial port (e.g. '123') to identify the job"
)
def stop_vllm(identifier, port):
    ...
    try:
        ...
        # 3. No args: try to stop the last started server from config
            ...
            if not servers:
                console.error(
                    "No servers configured. Use 'cmc llm start <name>' first."
                )
                return
            ...
            if last_server:
                if orchestrator.stop_uva(server_name=last_server):
                    console.ok(
                        f"Successfully stopped last started server: {last_server}."
                    )
                    return
